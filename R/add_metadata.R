...
#' Metadata fields remain list-columns if any of the selected workspace profiles
#' allow multiple values for that field, including in workspaces allowing only one value.
#'
#' @return A tibble.
#' @export
...
add_profile <- function(unit_items, units, md_profile, profiles, extra_columns = NULL) {
  ...
  # A shared column can be simplified only if every profile defining it allows
  # a single value. Otherwise preserve lists for all workspaces.
  profiles_single <-
    profiles_meta %>%
    dplyr::group_by(profile_name) %>%
    dplyr::summarise(single = all(!multiple), .groups = "drop") %>%
    dplyr::filter(single) %>%
