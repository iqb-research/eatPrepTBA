...
# Anonymize result
httr2_result <- purrr::map(httr2_result, \(x) { x[grepl("User$", names(x))] <- "Max Mustermann"; x[grepl("Changed|At$", names(x))] <- "2001-01-01T00:00:00.000Z"; x })
...
httr2_result[1]
```
transforms these nested lists into data frames or tibbles, so that the results are less nested, more consistent, and easier to use with common R workflows.
