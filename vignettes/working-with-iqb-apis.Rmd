...
The previous examples used `GET` requests to retrieve information from the Studio API. 

We now set `httr2::req_method("POST")`.
...
Additionally to the `access_token`, you also need to provide your user-ID for posting comments. It can be found in the developer tools payload tab.
...
In eatPrepTBA, users should usually not construct this request body manually. The package can use information from the Studio login, for example the logged-in user name, so that comments are not posted under an arbitrary user name.


## Changing existing data with PATCH requests

We now set `httr2::req_method("PATCH")`.

A `PATCH` request is used to change something that already exists. For example, changing the short name of a unit modifies the existing unit properties:
