...
example_file <- function(name) {
  local_file <- file.path("..", "inst", "extdata", name)
  if (file.exists(local_file)) return(local_file)
  system.file("extdata", name, package = "eatPrepTBA")
}
```
...
For a first connection, see the [Studio introduction](eatPrepTBA.html).
The examples below use placeholder credentials and example workspace and unit
IDs. Replace them with your own values. Screenshots show an earlier Studio
version; the saved metadata example has been adapted to the current format.
...
When reproducing the request in R with `httr2`, this value is passed with
`req_headers()`. In eatPrepTBA, [`login_studio()`](../reference/login_studio.html)
returns a `LoginStudio` object whose request function uses the token for later
requests; users do not need to copy it manually.

Set `app_version` once to the version shown by your Studio instance. It is not
detected automatically. For manual requests, the version is also visible in the
browser's `App-Version` request header.
...
httr2_result <- readr::read_rds(example_file("example_httr2_result.rds"))
```
...
```{r echo = FALSE}
# An anonymized saved response adapted to the current metadata format
properties <- readr::read_rds(example_file("example_properties.rds"))
```
...
`sourceVariableId` links an item to its variable; `sourceVariableUuid` stores the
variable UUID when available. eatPrepTBA exposes these as `variable_id` and
`variable_ref`. Older saved responses may still use `variableId` and
`variableReadOnlyId`; the package also accepts those fields.

Metadata values can be vocabulary selections, language-coded text, or simple
values with `raw` and `asText`. The following examples select entries by ID
within the example profile, rather than relying on their position.
...
```{r aufgabenzeit}
# Aufgabenzeit: stored value and formatted text
duration <- purrr::keep(entries, ~ .x$id == "iqb_time_unit")[[1]]
duration$value$raw
duration$value$asText[[1]]$value
```
...
Older metadata may have a separate `valueAsText` field. For routine analysis,
[`get_units()`](../reference/get_units.html) prepares both formats as
`unit_profiles`, `items_list`, and `items_profiles`, so these manual lookups
are not needed.
...
```{r echo = FALSE}
scheme_string <- readr::read_file(example_file("example_scheme_string.json"))
```
...
cat(c(head(scheme_lines, 20), "..."), sep = "\n")
```
